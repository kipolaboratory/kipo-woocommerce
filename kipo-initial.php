...
function init_kipopay_payment() {
	...
	/**
	 * Check kipo response is exist or not
	 */
	...
	class WC_full_kipopay extends WC_Payment_Gateway {
		...
		function check_kipo_response() {
			...
			if ( isset( $session['kipo-woocommerce'] ) AND ! empty( $session['kipo-woocommerce'] ) ) {
				...
				$payment_check = $kipo_gateway->verifyOrder( $this->merchant_key, $kipo_session['sk'] );

				if ( $payment_check['status'] ) {
					$this->msg['kipo_class']                  = 'success';
					$this->msg['kipo_message']                = 1;
					$this->msg['transaction_code']            = $payment_check['reference_id'];
					$_SESSION['order'][ $order_id ]['status'] = 'wc-processing';

					$order->payment_complete();
					$order->add_order_note( 'پرداخت موفق، کد پیگیری پرداخت: ' . $payment_check['reference_id'] );
		...
		public static function showMessage( $content ) {
			$kipo_message     = kipopayGatewayClass::kipoMessage( $_GET['kipo_message'] );
			...
			if ( $kipo_class === 'success' ) {
				...
				$return .= '<p class="kipopay-woocommerce-reference-code">کد پیگیری پرداخت شما <strong>' . $transaction_code . '</strong> می‌باشد. این کد را جهت پیگیری‌های بعدی لطفا ذخیره نمایید</p>';
				$return .= '</div>';
				$return .= $content;
			} else {
				$return = '<div class="kipopay-woocommerce box ' . $kipo_class . '-box">';
